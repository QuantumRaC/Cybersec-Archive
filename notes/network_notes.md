...
- **SSH (Secure Shell)**
  ...
  - key pairs generated by `ssh-keygen` program; supports various algorithms
    ...
    - when generating an SSH key to log into a remote machine, you should generate the key on local machine and copy the public key over using `ssh-copy-id` - private key should never exist on the target machine.
  ...
  - for usage see [`ssh-key-gen`](linux_notes.md#ssh-keygen)
...
- **Digital Signature**
  - digital signatures provides a way to verify the authenticity and integrity of a digital message or document with asymmetric cryptography

- **Certificates**
  - trusted by your browser by a long trust chain
  - e.g. cert signed by an organization, the organization is trusted by a root Certificate Authority, the CA is trusted by your browser
  - use Let's Encrypt (https://letsencrypt.org/) for free TLS certificates

- **PGP (Pretty Good Privacy)**
  - software that implements encryption for encrypting files, performing digital signing, etc.
  - GnuPG (GPG) is an open-source implementation of the OpenPGP standard
  - `gpg --full-gen-key`
  - GPG can be used to decrypt files in CTFs and protect passphrases like how SSH private keys are protected
